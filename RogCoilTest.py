...
def process_shot(shot_no, saveFig=True, fileFormat='png'):
    ...
    if shot_no <= 50260:
        ...
        MHD_data['U_loop MHD'] *= 10  # kvůli děliči napětí
        MHD_data['U_loop'] = U_loop_interp
...
shot_list = list(range(50245, 50260))
...
for shot, data in all_results.items():
    ...
    plt.plot(data['MHD_data']['t'], data['MHD_data']['U_loop']/np.max(abs(data['MHD_data']['U_loop'])), label=r'$U_{loop}$ basic')
    plt.plot(data['MHD_data']['t'], data['MHD_data']['U_loop MHD']/np.max(abs(data['MHD_data']['U_loop MHD'])), label=r'$U_{loop}$')
    ...
    plt.plot(data['int_data']['t'], data['int_data']['U_loop MHD']/np.max(abs(data['int_data']['U_loop MHD'])), label=r'$U_{loop}$' f' #{shot}')
...
for shot, data in all_results.items():
    ...
    plt.plot(data['MHD_data']['t'], data['MHD_data']['U_loop MHD']/np.max(abs(data['MHD_data']['U_loop MHD'])), label=r'$U_{loop}$' f' #{shot}')
...
for shot, data in all_results.items():
    ...
    # plt.plot(data['int_data']['t'], data['int_data']['rog'], label=f'rog #{shot}')
    # plt.plot(data['int_data']['t'], data['int_data']['U_loop']/np.max(abs(data['int_data']['U_loop'])), label=r'$U_{loop}$' f' #{shot}')
    # plt.plot(data['int_data']['t'], data['int_data']['Bt HFS']/np.max(abs(data['int_data']['Bt HFS'])), label=f'Bt HFS #{shot}')
    # plt.plot(data['int_data']['t'], data['int_data']['noise']/np.max(abs(data['int_d ata']['noise'])), label=f'noise #{shot}')
