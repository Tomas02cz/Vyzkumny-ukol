...
address_new_diag = 'http://golem.fjfi.cvut.cz/shots/{shot_no}/Diagnostics/MHDring-TM/{name}'# old coils (new for Godsfavour)
...
def open_new_diag(shot_no, name, url_template=address_new_diag):
    return ds.open(url_template.format(shot_no=shot_no, name=name))
...
t_plasma_start=get_scalar(shot_no, "t_plasma_start")*1e-3
t_plasma_end=get_scalar(shot_no,"t_plasma_end")*1e-3
plasma_endpoints= [t_plasma_start,t_plasma_end]

HFS= read_new_diag(shot_no,"U_Bt_HFS")
LFS=read_new_diag(shot_no,"U_Bt_LFS")
inner_coil= read_new_diag(shot_no,"U_diam_inner")
outer_coil=read_new_diag(shot_no,"U_diam_outer")
Ip = read_new_diag(shot_no,"U_rogowski")


### VACUUM SHOT
HFS_vac= read_new_diag(vac_shot_no,"U_Bt_HFS")
LFS_vac=read_new_diag(vac_shot_no,"U_Bt_LFS")
inner_coil_vac= read_new_diag(vac_shot_no,"U_diam_inner")
outer_coil_vac=read_new_diag(vac_shot_no,"U_diam_outer")
Ip_vac = read_new_diag(vac_shot_no,"U_rogowski")
...
def plot_magnetic_fields(shot_no, vac_shot_no):
    ...
    HFS_1= read_new_diag(shot_no,"U_Bt_HFS")
    LFS_1=read_new_diag(shot_no,"U_Bt_LFS")
    inner_coil= read_new_diag(shot_no,"U_diam_inner")
    outer_coil=read_new_diag(shot_no,"U_diam_outer")
    Ip = read_new_diag(shot_no,"U_rogowski")
...
# for f in glob.glob(os.path.join(tmp_dir, f"{shot_no}_*.png")):
#     try:
#         os.remove(f)
#     except OSError:
#         pass